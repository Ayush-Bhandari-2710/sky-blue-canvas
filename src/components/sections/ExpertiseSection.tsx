...
───────────────────────────────────────── */
const expertise = [
  { name: "Hybrid Cloud",              value: 80, icon: Cloud,        color: "#3b82f6" },
  { name: "DevSecOps",                 value: 80, icon: ShieldCheck,  color: "#8b5cf6" },
  { name: "SRE",                       value: 60, icon: Activity,     color: "#06b6d4" },
  { name: "Container Orchestration",   value: 70, icon: Boxes,        color: "#10b981" },
  { name: "Infrastructure Automation", value: 80, icon: Cog,          color: "#f59e0b" },
  { name: "Containers",                value: 70, icon: Container,    color: "#ef4444" },
  { name: "Quality Engineering",       value: 60, icon: CheckCircle2, color: "#ec4899" },
  { name: "Analytics",                 value: 60, icon: BarChart3,    color: "#14b8a6" },
];

const additionalGroupsRaw = [
  { title: "CI Engines",             icon: ServerCog, items: ["Jenkins","Hudson","Bamboo","Travis","Cruise-control","Atlas","GO-CD"] },
  { title: "Cloud Platforms",        icon: Globe,     items: ["AWS","GCP","Azure","Openstack","Softlayer","VMware"] },
  { title: "IPAM",                   icon: Database,  items: ["Nipap","PHPIPAM","GestoIP","Infoblox","Solarwinds IPAM","Device42"] },
  { title: "Deployment Tools",       icon: Layers,    items: ["Vagrant","Rightscale","Scalr","Enstratius","Otto","XLdeploy"] },
  { title: "Source Code Mgmt",       icon: GitBranch, items: ["Git","Gerrit","Gitlab","BitBucket","Subversion","Perforce"] },
  { title: "Provisioning",           icon: Package,   items: ["Crowbar","Razor","Cobbler","Kickstart","Terraform"] },
  { title: "Monitoring",             icon: Eye,       items: ["Prometheus","Nagios","Hyperic","Zabbix","Cacti"] },
  { title: "Packaging Systems",      icon: Package,   items: ["DEB","RPM","YUM","ISO"] },
  { title: "Libraries & Connectors", icon: Link2,     items: ["fog","jcloud","libcloud"] },
  { title: "Identity Management",    icon: KeyRound,  items: ["Vault","Centrify"] },
  { title: "ITSM Suite",             icon: Ticket,    items: ["BMC Remedy","ServiceNow"] },
  { title: "Issue Tracking",         icon: Ticket,    items: ["Jira","RedMine"] },
  { title: "Service Discovery",      icon: Network,   items: ["Consul","Serf"] },
  { title: "Blockchain",             icon: Link2,     items: ["Hyperledger Fabric"] },
];

const additionalGroups = [...additionalGroupsRaw].sort((a, b) => b.items.length - a.items.length);
const topTiles    = additionalGroups.slice(0,  4);
const leftTiles   = additionalGroups.slice(4,  7);
const rightTiles  = additionalGroups.slice(7,  10);
const bottomTiles = additionalGroups.slice(10, 14);
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        border: `1px solid ${hovered ? `${e.color}55` : "rgba(255,255,255,0.13)"}`,
    ...
    >
      ...
      <div style={{ position: "relative", zIndex: 1, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ width: 30, height: 30, borderRadius: 9, background: `${e.color}22`, border: `1px solid ${e.color}44`,
        ...
        {hovered && (
          <motion.div initial={{ opacity: 0, scale: 0.6 }} animate={{ opacity: 1, scale: 1 }}
            style={{ fontSize: 18, fontWeight: 800, color: e.color, lineHeight: 1 }}>
            {e.value}%
          </motion.div>
        )}
      </div>
      <div style={{ position: "relative", zIndex: 1, fontSize: 12, fontWeight: 600,
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        {!hovered && <div style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", marginTop: 3 }}>{e.value}%</div>}
      </div>
    </motion.div>
  );
}
...
/* ─────────────────────────────────────────
   SKILL TILE — self-contained popover
───────────────────────────────────────── */
  ...
  return (
    <>
      <motion.button
        ...
        className="stile"
        style={{
          minHeight: compact ? 74 : 90,
          position: "relative", overflow: "hidden",
          outline: popOpen ? `2px solid ${accentColor}35` : "none",
          outlineOffset: 2,
        }}
      ...
      >
          ...
          {(hovered || popOpen) && (
            <motion.div key="badge"
              ...
              style={{
                ...
                background: popOpen ? accentColor : `${accentColor}20`,
                border: `1px solid ${accentColor}50`,
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: 9, fontWeight: 800,
                color: popOpen ? "white" : accentColor,
        ...
        <motion.div
          style={{
            width: compact ? 28 : 32, height: compact ? 28 : 32, borderRadius: compact ? 8 : 9,
        ...
        <div style={{ fontSize: compact ? 10.5 : 11.5, fontWeight: 600, color: hovered ? "#111827" : "#1e2a55",
          lineHeight: 1.3, position: "relative", zIndex: 1, transition: "color 0.18s" }}>
          {g.title}
        </div>
      ...
      <AnimatePresence>
        {popOpen && (
          <motion.div
            ...
            style={{
              position: "fixed",
              top: pop.top, left: pop.left,
              width: 268, zIndex: 99999,
              background: "white",
              borderRadius: 14,
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 9 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontSize: 10, color: "#9aa5c8", fontWeight: 500, marginTop: 1 }}>{g.items.length} tools</div>
              </div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
              {g.items.map((it) => (
                <span key={it} style={{
                  ...
                  background: `${accentColor}08`, border: `1px solid ${accentColor}20`, color: "#3a4a7a",
                }}>
                  {it}
                </span>
              ))}
    ...
    <section id="expertise" ref={ref} style={{
      ...
      <style>{`
        ...
        .exp-tag {
          ...
          background: white; border: 1px solid rgba(99,130,255,0.2); color: #4a6df0;
          box-shadow: 0 2px 12px rgba(99,130,255,0.09);
        }
        .exp-title {
          font-size: clamp(28px, 3.5vw, 48px); font-weight: 800;
          letter-spacing: -0.025em; line-height: 1.1; color: #111827; margin: 0;
        }
        .exp-title em {
          font-style: normal;
        ...
        .intro-strip {
          background: linear-gradient(135deg, #1a2a6c 0%, #1e3a8a 52%, #1e40af 100%);
        ...
        .intro-strip::before {
          ...
          background:
            radial-gradient(ellipse 60% 80% at 92% 50%, rgba(99,160,255,0.18) 0%, transparent 60%),
            radial-gradient(ellipse 40% 60% at 5% 18%, rgba(120,80,255,0.14) 0%, transparent 55%);
        }
        .intro-strip::after {
          content: ''; position: absolute; inset: 0; pointer-events: none;
        ...
        .slabel {
          ...
          font-size: 12.5px; font-weight: 700; color: #1e2a55; margin-bottom: 14px;
        }
        ...
        .stile {
          ...
          box-shadow: 0 2px 12px rgba(60,80,180,0.05);
          padding: 13px 10px 11px; text-align: center; cursor: pointer;
          transition: border-color 0.2s, box-shadow 0.2s, background 0.2s;
          display: flex; flex-direction: column;
          align-items: center; gap: 5px; justify-content: center; width: 100%;
        }
      ...
      <div style={{ maxWidth: 1160, margin: "0 auto", padding: isMobile ? "0 16px" : isTablet ? "0 24px" : "0 32px" }}>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }}
          ...
          <p style={{ fontSize: 14, color: "#9aa5c8", marginTop: 10, fontWeight: 400 }}>
            Deep capabilities across cloud, automation, and platform engineering.
          </p>
        </motion.div>
        ...
        <motion.div className="intro-strip"
          ...
          style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr auto", gap: isMobile ? 24 : 48, alignItems: "center" }}>

          <div style={{ position: "relative", zIndex: 1 }}>
            <p style={{ fontSize: 15.5, lineHeight: 1.8, color: "rgba(255,255,255,0.75)", fontWeight: 300, maxWidth: 520, marginBottom: 28 }}>
              Navigating complex IT infrastructures with advanced automation and containerization
              to ensure optimal performance, scalability, and reliability at every layer.
            </p>
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 10 }}>
              {[
                { icon: TrendingUp, color: "#3b82f6", label: "99.9999% Uptime",      sub: "Availability-first architecture"  },
                { icon: Zap,        color: "#8b5cf6", label: "End-to-end Pipelines", sub: "CI/CD from code to production"    },
                { icon: Award,      color: "#06b6d4", label: "10+ Years Expertise",  sub: "Infrastructure & cloud platforms" },
                { icon: Users,      color: "#10b981", label: "Multi-cloud Native",   sub: "AWS · GCP · Azure · Openstack"    },
              ].map((s, si) => (
          ...
          {!isMobile && (
            <div style={{ position: "relative", zIndex: 1, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, minWidth: 310 }}>
        ...
        {/* BENTO LABEL */}
        <motion.div initial={{ opacity: 0, y: 10 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.2 }} style={{ marginBottom: 14 }}>
          <div className="slabel"><span className="slabel-dot" />Tooling & Skills</div>
        </motion.div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={isInView ? { opacity: 1 } : {}} transition={{ duration: 0.4, delay: 0.14 }}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(8, 1fr)", gap: 12 }}>
              ...
              <div style={{ gridColumn: "3 / 7", display: "grid", gridTemplateColumns: "1fr 1fr", gridTemplateRows: "1fr 1fr", gap: 12 }}>
                {[
                  { label: "Infrastructure as Code", icon: Cog,         color: "#f59e0b", sub: "Chef · Puppet · Ansible · Terraform" },
                  { label: "Container Ecosystem",    icon: Container,   color: "#ef4444", sub: "Docker · Kubernetes · Nomad · LXC"   },
                  { label: "Security & Compliance",  icon: ShieldCheck, color: "#8b5cf6", sub: "DevSecOps · Vault · Policy gates"     },
                  { label: "Observability",          icon: Eye,         color: "#06b6d4", sub: "Prometheus · ELK · Splunk · Zabbix"   },
                ].map((c, ci) => (
